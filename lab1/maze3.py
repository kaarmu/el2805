...
class Maze:
    ...
    def _state2str(self, state):
        table = {
            ...
            self.STATE_NO_KEY: 'no key',
            self.STATE_HAS_KEY: 'has key',
        }
        return (
    ...
    def _states(self):
        ...
        Tuple ::= (Coord)               # if not enabled_minotaur
                ...
                | (Coord, Coord, Key)  # if enabled_key

        Coord ::= (Int, Int)            # rows, cols

        Key   ::= STATE_NO_KEY        # if enabled_key
                | STATE_HAS_KEY 
        ...
        key = (self.STATE_NO_KEY, self.STATE_HAS_KEY)
        args += [key] if self.enabled_key else []
    ...
    def _apply(self, state, action):
        ...
        if self.enabled_minotaur:
            ...
            for next_minotaur in all_next_minotaur:
                ...
                elif self.enabled_key:
                    for key_position in zip(*np.where(self.maze == self.MAZE_KEY)):
                        if next_player == key_position:
                            next_states.append((next_player, next_minotaur, self.STATE_HAS_KEY))
                        else:
                            next_states.append((next_player, next_minotaur, state[2]))
                else:
